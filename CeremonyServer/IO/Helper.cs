...
namespace Ceremony
{
    public static class Helper
    {
        ...
        public static byte[] HexToBytesInverse(this string value)
        {
            if (value == null || value.Length == 0)
                return new byte[0];
            if (value.Length % 2 == 1)
            ...
            byte[] result = new byte[value.Length / 2];
            for (int i = 0; i < result.Length; i++)
                result[result.Length - i - 1] = byte.Parse(value.Substring(i * 2, 2), NumberStyles.AllowHexSpecifier);
            return result;
        }
        ...
        public static bool ByteCompare(this IEnumerable<byte> val1, IEnumerable<byte> val2)
        {
            ...
            {
                if (val1.ElementAt(i) != val2.ElementAt(i))
                {
                    return false;
                }
            }
        ...
        public static string ToHexString(this List<byte[]> value)
        {
            StringBuilder sb = new StringBuilder();
            foreach (IEnumerable<byte> item in value)
            {
